...
def compute_decoder_losses(recon: torch.Tensor, target: torch.Tensor, mse_weight: float) -> dict[str, torch.Tensor]:
    ...
    *,
    ...
    for batch_idx, batch in enumerate(progress, start=1):
        ...
        with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_dtype is not None):
            ...
            losses = compute_decoder_losses(recon, target, decoder_config["mse_weight"])
